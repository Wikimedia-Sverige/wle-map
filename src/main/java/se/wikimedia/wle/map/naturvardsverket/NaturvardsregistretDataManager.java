...
import se.wikimedia.wle.map.Wikidata;
import se.wikimedia.wle.map.WikimediaCommons;
...
import se.wikimedia.wle.map.prevalence.queries.GetObjectByQ;
import se.wikimedia.wle.map.prevalence.queries.GetPreviousSuccessfulNoValuesPollStarted;
import se.wikimedia.wle.map.prevalence.queries.GetPreviousSuccessfulPollStarted;
import se.wikimedia.wle.map.prevalence.queries.GetWikimediaImage;
import se.wikimedia.wle.map.prevalence.transactions.*;
...
@Service
public class NaturvardsregistretDataManager extends AbstractLifecycle implements SmartLifecycle {
    ...
    private final WikimediaCommons commons;
    private final NaturvardsregistretIndex index;
    private final Clock clock;

    @Autowired
    public NaturvardsregistretDataManager(
    ...
    private void sleep(long millis) throws InterruptedException {
        ...
        while (!isStopping() && System.currentTimeMillis() < end) {
    ...
    public void doStart() throws Exception {
        ...
        pollUpdatedWikidataEntriesThread.setName("Wikidata poller");
